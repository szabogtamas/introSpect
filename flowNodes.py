...
class nextflowProcess:
    ...
    def __init__(self, command=None, params=None, inchannels=None, outchannels=None, inputs=None, outputs=None, conda=None, container=None, process_settings=None, capture=False, capturepars=['notebooktitle', 'capturednotebook'], modified_kws={"outFile": (1, "-o", "--outFile", {"dest": "outFile", "help": "Location where results should be saved. If not specified, STDOUT will be used.",},),}, manualDoc=None):
        ...
        if manualDoc is None:
            manualDoc = '' # If anything is to be added to the description of the process
        self.manualDoc = manualDoc
        if modified_kws is None:
            modified_kws = dict() # Parameters added to argparse of the function. Output file names, for example.
        self.capture = capture # Converts the process into markdown of a notebook (easily modify plots)
        self.capturepars = capturepars
        self.modified_kws = modified_kws # Keywords controling the outputs of the process function
        self.cmdpars = None
        self.cmdouts = None
        self.cmdpreset = dict()
        self.addedparams = []
        ...
        if self.inputs is None:
            ...
            for k, v in specified_channels.items():
                if k in self.inchannels:
                    ...
                    inputs += v[0] + ' ' + channelVariable + channelSource + channelTransform + '\n'
            ...
            for k, v in specified_channels.items():
                if k in self.outchannels:
                    ...
                    if v[3] is None:
                        ...
                        channelTransform = v[3]
                    out += v[0] + ' ' + channelVariable + channelName  + channelTransform + '\n'
            for k, v in remainder.items():
                out += 'val ' + k + ' into '+k+'\n'
                if k in self.cmdpars:
                    cm = self.cmdpars[k]
    ...
    def compile_process(self, dr):
        if self.command is None:
            ...
            if self.inputs is None:
                raise ValueError('The command does not seem to be a python function, so we cannot guess its inputs. Please supply a list of inputs channel statements!!!')
        ...
        if self.container in [None, '']:
            if 'container' in dependencies and dependencies[container] not in [None, '']:
    ...
    def compile_command(self):
        ...
        if self.capture:
            command += '\n            captureIntoNotebook.py --notebooktitle $'+notebooktitle+' --capturednotebook $'+capturednotebook + ' ' + command
        return command
    ...
    def generate_py(self, fn, dr):
        dependencies = {'conda': '', 'container': '', 'imports': [], 'git_packages': [], 'inhouse_packages': []}
        dependencies.update(self.dependencies())
        nonCondaCopy(dependencies['git_packages'], dependencies['inhouse_packages'], dr+'/packages')
...
def channelNodes(*args, location='', main_kws=None, generalSettings=None, containerPaths=None, verbose=True):
    ...
    for k, v in process_settings.items():
        if v is not None:
            ...
            processSettings += s + '    }\n'
    processSettings += '}\n'
    configBody = '\n'.join(mainparams) + '\n\n'  + textwrap.dedent(general_cluster_profile) + processSettings + textwrap.dedent(generalSettings)
...
def endScriptConneted(f, modified_kws):
    connected = """
    def main():
        mainFunction = introSpect.commandLines.cmdConnect("""+f+""", """+str(modified_kws)+""")
        mainFunction.eval()
        mainFunction.save()
        return
...
def cleanup(location=os.getcwd()):
